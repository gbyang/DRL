...
def get_traj(agent, env, episode_max_length):
    ...
    ob = env.observe()
    ob_random = env_random.observe() ###

    num_ma = env.pa.num_machines # 保存避免以便重复调用
    num_max = env.pa.num_nw # 保存避免以便重复调用
    for _ in xrange(episode_max_length):
        compare_rews = {} # 比较rewards
        # ----------------------- #
        #       由reward获得a      #
        # ----------------------- #
        ...
        #          随机获得a       #
        # ----------------------- #
        num_act = num_ma * num_max + 1  # if no action available,
        a_random = np.random.randint(num_act) ###

        obs.append(ob)  # store the ob at current decision making step
        obs_random.append(ob_random)
        acts.append(a)
        acts_random.append(a_random)
        ob, rew, done, info = env.step(a, repeat=True)
        ob_random, rew_random, done_random, info_random = env_random.step(a_random, repeat=True)
        ...
        # 轮流放入各机器得到reward
        # env_copy = [] # 测试环境（每个资源池复制一个）
        # for i in xrange(num_ma):
        #     env_copy.append(copy.deepcopy(env))  # 复制一个环境作为测试
        #     if a == num_ma * num_max: # 无效操作无需比较放入哪个机器
        #         ob, rew, done, info = env.step(a, repeat=True)
        #         break
        #     else: # 有效操作轮流放入机器
        #         choose = (a + num_max * i)
        #         if choose >= (num_ma * num_max):
        #             choose -= (num_ma * num_max)
        #         ob, rew, done, info = env_copy[i].step_test(choose, repeat=True) # 使用复制的
        #         compare_rews[choose] = rew # 保存数组最大值
        #
        # # 若action有效，比较得到最高的reward
        # if compare_rews:
        #     best_choose = max(compare_rews, key=compare_rews.get) # 获取reward最好的action
        #     ob, rew, done, info = env.step(best_choose, repeat=True) # 将最好的action放入执行环境
        #     acts.append(best_choose)  # TODO 是否保存修改后的choose
        # else:
        #     acts.append(a)

        rews.append(rew)
        rews_random.append(rew_random)
        entropy.append(get_entropy(act_prob))

        if done: break

    return {'reward': np.array(rews),
            'reward_random': np.array(rews_random),
            'ob': np.array(obs),
            'ob_random': np.array(obs_random),
            'action': np.array(acts),
            'action_random': np.array(acts_random),
            'entropy': entropy,
            'info': info,
            'info_random': info_random
            }
...
def process_all_info(trajs):
    ...
    for traj in trajs:
        ...
        enter_time_random.append(np.array([traj['info_random'].record[i].enter_time for i in xrange(len(traj['info_random'].record))]))
        finish_time.append(np.array([traj['info'].record[i].finish_time for i in xrange(len(traj['info'].record))]))
        finish_time_random.append(np.array([traj['info_random'].record[i].finish_time for i in xrange(len(traj['info_random'].record))]))
        job_len.append(np.array([traj['info'].record[i].len for i in xrange(len(traj['info'].record))]))
        job_len_random.append(np.array([traj['info_random'].record[i].len for i in xrange(len(traj['info_random'].record))]))

    enter_time = np.concatenate(enter_time)
    enter_time_random = np.concatenate(enter_time_random)
    finish_time = np.concatenate(finish_time)
    finish_time_random = np.concatenate(finish_time_random)
    job_len = np.concatenate(job_len)
    job_len_random = np.concatenate(job_len_random)

    return enter_time, finish_time, job_len, enter_time_random, finish_time_random, job_len_random


def plot_lr_curve(output_file_prefix,
                  max_rew_lr_curve, mean_rew_lr_curve, slow_down_lr_curve,
                  max_rew_lr_curve_random, mean_rew_lr_curve_random, slow_down_lr_curve_random,
    ...
    # ax.plot(mean_rew_lr_curve_random, linewidth=2, label='PG_random mean')
    ...
    # ax.plot(max_rew_lr_curve_random, linewidth=2, label='PG_random max')
    ...
    # ax.plot(slow_down_lr_curve_random, linewidth=2, label='PG_random mean')
    for k in ref_discount_rews:
...
def get_traj_worker(pg_learner, env, pa, result):
    ...
    all_eprews_random = np.array([discount(traj["reward_random"], pa.discount)[0] for traj in trajs])  # episode total rewards
    all_eplens = np.array([len(traj["reward"]) for traj in trajs])  # episode lengths

    # All Job Stat
    enter_time, finish_time, job_len, enter_time_random, finish_time_random, job_len_random = process_all_info(trajs)
    finished_idx = (finish_time >= 0)
    finished_idx_random = (finish_time_random >= 0)
    all_slowdown = (finish_time[finished_idx] - enter_time[finished_idx]) / job_len[finished_idx]
    all_slowdown_random = (finish_time_random[finished_idx_random] - enter_time_random[finished_idx_random]) / job_len_random[finished_idx_random]
    ...
    result.append({"all_ob": all_ob,
                   ...
                   "all_eprews_random": all_eprews_random,
                   "all_eplens": all_eplens,
                   "all_slowdown": all_slowdown,
                   "all_slowdown_random": all_slowdown_random,
...
def launch(pa, pg_resume=None, render=False, repre='image', end='no_new_job'):
    ...
    mean_rew_lr_curve = []
    mean_rew_lr_curve_random = []
    max_rew_lr_curve = []
    max_rew_lr_curve_random = []
    slow_down_lr_curve = []
    slow_down_lr_curve_random = []
    ...
    for iteration in xrange(1, pa.num_epochs):
        ...
        for ex in xrange(pa.num_ex):
            ...
            if ex_counter >= pa.batch_size or ex == pa.num_ex - 1:
                ...
                all_eprews_random.extend([r["all_eprews_random"] for r in result])

                eprews.extend(np.concatenate([r["all_eprews"] for r in result]))  # episode total rewards
                eprews_random.extend(np.concatenate([r["all_eprews_random"] for r in result]))  # episode total rewards
                ...
                all_slowdown_random.extend(np.concatenate([r["all_slowdown_random"] for r in result]))
        ...
        max_rew_lr_curve_random.append(np.average([np.max(rew_random) for rew_random in all_eprews_random]))
        mean_rew_lr_curve.append(np.mean(eprews))
        mean_rew_lr_curve_random.append(np.mean(eprews_random))
        slow_down_lr_curve.append(np.mean(all_slowdown))
        slow_down_lr_curve_random.append(np.mean(all_slowdown_random))
