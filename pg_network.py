...
def rmsprop_updates(grads, params, stepsize, rho=0.9, epsilon=1e-9): # rmsprop 梯度下降参数更新
...
def adam_update(grads, params, learning_rate=0.001, beta1=0.9, # adam优化梯度下降算法
...
class PGLearner:
    def __init__(self, pa):
        ...
        self.input_height = pa.network_input_height # 输入高度
        self.input_width = pa.network_input_width # 输入宽度
        self.output_height = pa.network_output_dim # 输出高度

        self.num_frames = pa.num_frames

        self.update_counter = 0

        states = T.tensor4('states') # 四维数组 states
        actions = T.ivector('actions') # int类型一维向量 actions
        values = T.vector('values') # 一维数组 values
        ...
        self.l_out = \
            build_pg_network(pa.network_input_height, pa.network_input_width, pa.network_output_dim) # 网络输出层

        # compact representation
        # self.l_out = \
        #     build_compact_pg_network(pa.network_input_height, pa.network_input_width, pa.network_output_dim)

        self.lr_rate = pa.lr_rate # 学习率
        self.rms_rho = pa.rms_rho
        self.rms_eps = pa.rms_eps

        params = lasagne.layers.helper.get_all_params(self.l_out) # 网络的所有参数
        ...
        loss = T.log(prob_act[T.arange(N), actions]).dot(values) / N  # call it "loss" 损失函数

        grads = T.grad(loss, params) # 损失函数，梯度求导

        updates = rmsprop_updates(
            grads, params, self.lr_rate, self.rms_rho, self.rms_eps) #梯度下降更新方法
    ...
    # get the action based on the estimated value 根据预测值选取action
...
def build_pg_network(input_height, input_width, output_length):
    ...
    l_in = lasagne.layers.InputLayer( # 输入层
        shape=(None, 1, input_height, input_width), #(批处理大小，通道，行，列)。batchsize设置为None，意味着网络将在编译后接受任意batchsize的输入数据。
    )

    l_hid = lasagne.layers.DenseLayer( # 隐藏层
        l_in, # 传入层
        num_units=20, # 全连接层的单元数量
        # nonlinearity=lasagne.nonlinearities.tanh,
        nonlinearity=lasagne.nonlinearities.rectify, # 非线性函数使用的是 rectify( = max(0, x))
        # W=lasagne.init.Normal(.0201),
        W=lasagne.init.Normal(.01), # 权重矩阵W的初始化器
        b=lasagne.init.Constant(0)
    )

    l_out = lasagne.layers.DenseLayer( # 输出层
        l_hid,
        num_units=output_length,
        nonlinearity=lasagne.nonlinearities.softmax,
        # W=lasagne.init.Normal(.0001),
